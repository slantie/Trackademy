...
class CourseService {
  public async create(data: CourseCreateData): Promise<Course> {
    ...
    if (existingCourse && !existingCourse.isDeleted) {
      throw new AppError("This exact course allocation already exists.", 409);
    }
  ...
  public async getAll(options: CourseQueryOptions = {}) {
    ...
    else if (sortBy === "faculty") orderBy.faculty = { fullName: sortOrder };
  ...
  public async getBySemester(
    ...
    return prisma.course.findMany({
      ...
      orderBy: [{ subject: { name: "asc" } }, { faculty: { fullName: "asc" } }],
      take: limit,
    });
  }
  ...
  public async getByStudentId(studentId: string): Promise<Course[]> {
  ...
  public async hardDelete(id: string): Promise<void> {
    ...
    const totalRelations =
      (course._count?.Attendance || 0) + (course._count?.enrollments || 0);
