...
export class StudentController {
    ...
    static async searchStudents(
        ...
        try {
            const { q } = req.query;
            const { departmentId, semesterId, divisionId, batch, limit } =
                req.query;
    ...
    static async getStudentsBySemester(
        ...
        try {
            ...
            const students = await studentService.getBySemester(
                semesterId,
                options
            );
    ...
    static async getStudentByIdWithRelations(
        ...
        try {
            const student = await studentService.getByIdWithRelations(
                req.params.id
            );
            res.status(200).json({
                status: "success",
