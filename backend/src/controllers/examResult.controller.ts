...
export class ExamResultController {
  ...
  static async searchExamResults(
    ...
    try {
      const { q, examId, semesterId, departmentId, status, limit } = req.query;
      ...
      const results = await examResultService.search(q as string, options);
      res.status(200).json({
        status: "success",
  ...
  static async getTopPerformers(
    ...
    try {
      const { examId, semesterId, departmentId, limit, byField } = req.query;
  ...
  static async getResults(
    ...
    try {
      ...
      if (user.role === Role.ADMIN || user.role === Role.FACULTY) {
        ...
        if (!examId)
          throw new AppError("Exam ID is required for this role.", 400);
        ...
        results = await examResultService.getByStudent(studentProfile.id);
      }
  ...
  static async getStatistics(
    ...
    try {
      const { examId, semesterId, departmentId, academicYearId } = req.query;
  ...
  static async getMyResults(
    ...
    try {
      ...
      const results = await examResultService.getByStudent(studentProfile.id);
