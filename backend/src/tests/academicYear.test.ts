...
describe("Academic Year API Endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    const college = await createTestCollege(TEST_COLLEGE_NAME);
  ...
  it("should update an academic year to be active and then fetch it", async () => {
    ...
    expect(getActiveResponse.body.data.academicYear.id).toBe(newAcademicYearId);
  });
  ...
  it("should restore a soft-deleted academic year when creating with the same year", async () => {
    ...
    expect(createResponse.body.data.academicYear.id).toBe(newAcademicYearId);
