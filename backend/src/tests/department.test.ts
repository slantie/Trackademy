...
describe("Department API Endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    // Create test college for department tests
    ...
    testCollegeId = collegeResponse.body.data.college.id;
  });
  ...
  describe("Department Search", () => {
    ...
    it("should filter search by college", async () => {
      const response = await request(app)
        .get(`/api/v1/departments/search?q=Test&collegeId=${testCollegeId}`)
        .set("Authorization", `Bearer ${adminToken}`);
