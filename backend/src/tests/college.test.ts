...
describe("College API Endpoints", () => {
  ...
  it("should create a college with contact information", async () => {
    const collegeData = {
      ...
      address: "123 Education Street, Learning City, Knowledge State 12345",
    ...
    await cleanupTestData({ colleges: ["Test College with Contact"] });
  });
  ...
  it("should get all colleges, including the newly created one", async () => {
    ...
    const names = response.body.data.colleges.data.map((c: any) => c.name);
  ...
  // -- 7. CLEANUP --
  afterAll(async () => {
    // Clean up all test colleges created during this test suite
    await cleanupTestData({
      colleges: [TEST_COLLEGE_NAME, "Test College with Contact"],
    });
  });
});
